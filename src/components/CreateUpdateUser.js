...
class CreateUpdateUser extends Component {
  ...
  handleSaveClicked = () => {
    ...
    if (isAdding) {
      if (avatarFile) {
        this.imagesApi.add({id, images: [avatarFile]})
          .then(result => {
...
export default withMessage(withUserContext(withRouter(withStyles(styles)(CreateUpdateUser))));
